...
void Database::Connect(Database::Engine type, const std::string& host, unsigned short port, const std::string& user, const std::string& pass, const std::string& db)
{
	...
	case MySQL:
		...
		if (mysql_get_client_version() != MYSQL_VERSION_ID)
		{
			...
			Console::Err("MySQL client library version mismatch! Please recompile the ARCOSERV with the correct MySQL client library.");
			...
			Console::Err("Make sure the ARCOSERV is using the correct version of libmariadb.dll");
		...
		// This check isn't really neccessary for ARCOSERV
#if 0
